...
"""Uniqueness pillar bar chart showing numeric cardinality and discreteness, with top-N coverage."""
...
@dataclass
class UniquenessCardinalityBarContext(SeriesBarChartContext):
    """Context/config for the uniqueness cardinality bar plot (axes, sorting, thresholds)."""
...
class UniquenessCardinalityBarPlot(SeriesBarChartMixin, BasePlot):
    """
    ...
    def __init__(self, ctx: UniquenessCardinalityBarContext):
    ...
    def _format_label(self, v: Any) -> str:
        """Normalize value labels for display."""
        try:
            return str(v)
        except Exception:
            return repr(v)
    ...
    def compute_descriptive(self, s: pd.Series) -> dict[str, Any]:
        ...
        full_counts = {self._format_label(k): v for k, v in s.value_counts().to_dict().items()}
        ...
        named_counts = [int(bars[k]["count"]) for k in bars.keys() if (k != other_display)]
