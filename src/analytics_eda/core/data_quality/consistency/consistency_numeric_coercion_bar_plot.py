...
"""Consistency pillar: numeric-coercion bar chart for non-numeric tokens."""
...
@dataclass
class ConsistencyNumericCoercionBarContext(SeriesBarChartContext):
    ...
    title_template: str = "Numeric Coercion Issues for {name}{modifiers}"
...
class ConsistencyNumericCoercionBarPlot(SeriesBarChartMixin, BasePlot):
    """
    ...
    def compute_descriptive(self, s: pd.Series) -> dict[str, Any]:
        ...
        # Treat datetime-like values as coercible (exclude them from non-numeric bucket)
        non_numeric_mask = non_numeric_mask & ~s.map(lambda v: isinstance(v, pd.Timestamp | datetime.datetime | datetime.date | np.datetime64))
