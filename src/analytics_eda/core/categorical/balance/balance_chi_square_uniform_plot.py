...
@dataclass
class BalanceChiSquareUniformPlotContext(PlotContext):
...
class BalanceChiSquareUniformPlot(BasePlot):
    ...
    def __init__(self, ctx):
        parts = PlotParts(series_validator=categorical_validator())
    ...
    def draft_descriptive_findings(self, desc: dict[str, Any]) -> dict[str, Any]:
        ...
        secondary = f"Largest absolute deviation: {parts[0]}." if len(parts) == 1 else ("Largest deviations (tie): " + ", ".join(parts) + ".") if len(parts) > 1 else None
    ...
    def compute_inferential(self, s: pd.Series, desc: dict[str, Any]) -> dict[str, Any]:
        ...
        if any(e <= 0 for e in expected):
            return {"chi2_gof_null_uniform": {"warning": "Test not computed: at least one expected cell count is 0 (violates chi-square requirements)."}}
    ...
    def draft_inferential_findings(self, inf: dict[str, Any], desc: dict[str, Any]) -> dict[str, Any]:
        ...
        decision = "Frequencies differ from a uniform distribution" if reject else "No statistically significant deviation from uniform"
        ...
        if warning:
            findings["secondary_finding"] = f"{findings['secondary_finding']} Assumption warning: {warning}" if findings["secondary_finding"] else f"Assumption warning: {warning}"
