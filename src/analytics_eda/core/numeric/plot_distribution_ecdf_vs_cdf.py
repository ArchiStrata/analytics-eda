...
@dataclass
class ECDFvCDFContext(PlotContext):
...
class ECDFvsCDFNumericPlot(NumericSeriesMixin, BasePlot):
    """
...
def plot_distribution_ecdf_vs_cdf(
    ...
    /,
    *,
    ctx: Optional[ECDFvCDFContext] = None,
    **kwargs: Dict[str, Any],
) -> Dict[str, Any]:
    """
    ...
    - Otherwise we construct ECDFvCDFContext(**kwargs).

    NOTE: Provide distribution_name='norm'|'lognorm'|'gamma'|'expon' via kwargs
          (or set it on ctx) to mirror the legacy function signature.
    """
    if ctx is None:
        ctx = ECDFvCDFContext(**kwargs)
    else:
        for k, v in kwargs.items():
            setattr(ctx, k, v)

    plot = ECDFvsCDFNumericPlot(ctx)
