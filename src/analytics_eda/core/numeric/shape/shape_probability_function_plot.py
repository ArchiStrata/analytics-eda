...
@dataclass
class ShapeProbabilityFunctionContext(PlotContext):
...
class ShapeProbabilityFunctionPlot(BasePlot):
    """
    ...
    def metadata_overrides(self, *, series=None, cols=None, role_map=None) -> dict[str, Any]:
        ...
        ylabel = self.ctx.ylabel if self.ctx.ylabel is not None else ("Probability P(X = x)" if is_disc else "Density f(x)")
