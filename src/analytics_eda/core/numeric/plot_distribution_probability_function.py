...
@dataclass
class ProbFuncContext(PlotContext):
...
class ProbabilityFunctionPlot(NumericSeriesMixin, BasePlot):
    """
...
def plot_distribution_probability_function(
    ...
    /,
    *,
    ctx: Optional[ProbFuncContext] = None,
    **kwargs: Dict[str, Any],
) -> Dict[str, Any]:
    """
    ...
    """
    if ctx is None:
        ctx = ProbFuncContext(**kwargs)
    else:
        for k, v in kwargs.items():
            setattr(ctx, k, v)

    plot = ProbabilityFunctionPlot(ctx)
