...
from analytics_eda.core.numeric import DistributionECDFvsCDFContext, DistributionECDFvsCDFPlot
...
def test_validate_numeric_named_series_errors(series_factory, expected_exc, match):
    ...
    with pytest.raises(expected_exc, match=match):
        ctx = DistributionECDFvsCDFContext(distribution_name="norm")
        plot = DistributionECDFvsCDFPlot(ctx)
...
def test_invalid_distribution_name_raises_value_error():
    ...
    with pytest.raises(ValueError):
        ctx = DistributionECDFvsCDFContext(distribution_name="invalid")
        plot = DistributionECDFvsCDFPlot(ctx)
...
def test_distribution_ecdf_vs_cdf_plot_data_driven(make_series, kwargs, expect, tmp_path, assert_plot_metadata):
    ...
    ctx = DistributionECDFvsCDFContext(**kwargs)
    plot = DistributionECDFvsCDFPlot(ctx)
