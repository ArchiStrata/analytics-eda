...
from analytics_eda.core.numeric.shape import ShapeDensityContext, ShapeDensityPlot
...
def test_validate_numeric_named_series_errors(series_factory, expected_exc, match):
    ...
    with pytest.raises(expected_exc, match=match):
        ctx = ShapeDensityContext()
        plot = ShapeDensityPlot(ctx)
    ...
    [
        ...
        (
            ...
            {},
            {
                ...
                "descriptive_stats": {
                    "n": 5,
                    "quartile_skew": (lambda v, s=test_data_series: math.isclose(v, (s.quantile(0.75) + s.quantile(0.25) - 2 * s.median()) / (s.quantile(0.75) - s.quantile(0.25)), rel_tol=1e-12, abs_tol=1e-12)),
...
def test_distribution_density_plot_data_driven(make_series, kwargs, expect, tmp_path, assert_plot_metadata):
    ...
    ctx = ShapeDensityContext(**kwargs)
    plot = ShapeDensityPlot(ctx)
