...
from analytics_eda.core.numeric.shape import ShapeECDFvsCDFContext, ShapeECDFvsCDFPlot
...
def test_validate_numeric_named_series_errors(series_factory, expected_exc, match):
    ...
    with pytest.raises(expected_exc, match=match):
        ctx = ShapeECDFvsCDFContext(distribution_name="norm")
        plot = ShapeECDFvsCDFPlot(ctx)
...
def test_invalid_distribution_name_raises_value_error():
    ...
    with pytest.raises(ValueError):
        ctx = ShapeECDFvsCDFContext(distribution_name="invalid")
        plot = ShapeECDFvsCDFPlot(ctx)
...
def test_shape_ecdf_vs_cdf_plot_data_driven(make_series, kwargs, expect, tmp_path, assert_plot_metadata):
    s = make_series()
    ...
    ctx = ShapeECDFvsCDFContext(**kwargs)
    plot = ShapeECDFvsCDFPlot(ctx)
